#!/usr/bin/env python3
"""
Server DNS minimal pentru Săptămâna 10.
Răspunde la interogări A cu mapping-uri statice.
...
def parse_qname(data: bytes, offset: int) -> Tuple[str, int]:
    """Parsează numele din interogarea DNS."""
    labels = []
    while True:
        if offset >= len(data):
            return "", offset
        ...
        if length == 0:
            break
        # Verificare pointer (compresie DNS)
        if (length & 0xC0) == 0xC0:
...
def build_response(request: bytes, ip_map: Dict[str, str]) -> bytes | None:
    """Construiește răspuns DNS pentru cererea dată."""
    if len(request) < 12:
        return None
...
def main() -> int:
    ...
    parser.add_argument("--a", action="append", default=[], 
                        help="Mapping A: nume=ip (ex: api.local.=10.0.0.20)")
    args = parser.parse_args()
    
    # Construire dicționar de mapping-uri
    ip_map: Dict[str, str] = {}
    for item in args.a:
        if "=" in item:
            name, ip = item.split("=", 1)
            # Normalizare: adaugă punct final dacă lipsește
    ...
    while True:
        try:
            ...
            # Parse query name pentru logging
