#!/usr/bin/env python3
"""
Topologie Mininet pentru Servicii de Rețea - Seminar 10

Această topologie extinsă creează o infrastructură completă
pentru testarea serviciilor DNS, HTTP și FTP.
...
class ServicesTopo(Topo):
    """
    Topologie cu servicii pre-configurate (DNS, Web, FTP).
    """
    ...
    def build(self):
        info('*** Building services topology\n')
...
def create_dns_script() -> str:
    """Creează script Python pentru server DNS minimal."""
...
def create_web_server_script() -> str:
    """Creează script pentru server web simplu."""
    script = '''#!/usr/bin/env python3
...
def start_services(net: Mininet) -> dict:
    """
    Pornește serviciile pe host-uri.
    ...
    # FTP Server pe h3 (simplificat cu pyftpdlib dacă disponibil)
    h3 = net.get('h3')
    h3.cmd('mkdir -p /tmp/ftp')
    h3.cmd('echo "Welcome to FTP" > /tmp/ftp/welcome.txt')
    # Încercăm pyftpdlib, altfel Python built-in
...
def test_dns(net: Mininet) -> bool:
    """Testează serverul DNS."""
    info('\n*** Testing DNS\n')
    
    h4 = net.get('h4')
    
    # Test cu dig dacă disponibil
