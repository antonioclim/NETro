#!/usr/bin/env bash
#==============================================================================
# run_all.sh – Demo automat pentru Săptămâna 10 (Servicii de Rețea în Containere)
# 
# Rulează fără input interactiv, produce:
#   - artifacts/demo.log       (log complet al execuției)
#   - artifacts/demo.pcap      (captură trafic)
#   - artifacts/validation.txt (verificare automată)
#
# Rețele de Calculatoare, ASE București 2025-2026
# Revolvix&Hypotheticalandrei | Licență MIT
#==============================================================================
set -uo pipefail  # Continuăm la erori pentru a colecta toate rezultatele
...
# Porturi servicii (conform plan unitar)
DNS_PORT=5353
SSH_PORT=2222
FTP_PORT=2121
HTTP_PORT=8000
...
check_prerequisites() {
    ...
    if ! docker info &>/dev/null 2>&1; then
        log_error "Docker daemon nu rulează"
        ((errors++))
    fi
    
    if [ $errors -gt 0 ]; then
        log_error "Prerequisite lipsă: $errors erori"
        return 1
    fi
    
    log_info "Toate prerequisitele sunt îndeplinite"
    return 0
}
...
#------------------------------------------------------------------------------
setup_artifacts() {
    log_step "Pregătire directoare artefacte..."
    ...
    # Inițializare log
    echo "# Demo Log - Săptămâna 10" > "$LOG_FILE"
    echo "# Timestamp: $(date)" >> "$LOG_FILE"
    echo "# Porturi: DNS=$DNS_PORT SSH=$SSH_PORT FTP=$FTP_PORT HTTP=$HTTP_PORT" >> "$LOG_FILE"
    echo "" >> "$LOG_FILE"
    
    log_info "Artefacte pregătite în $ARTIFACTS_DIR/"
}
...
start_docker_services() {
    log_step "Pornire infrastructură Docker..."
    
    cd "$DOCKER_DIR"
    
    # Oprire eventuală existentă
    ...
    # Așteptare pentru healthy
    log_info "Așteptare servicii să devină healthy (max 60s)..."
    local attempts=0
    ...
    while [ $attempts -lt $max_attempts ]; do
        ...
        if [ "$healthy_count" -ge 3 ]; then
            log_info "Servicii healthy: $healthy_count"
            break
        fi
...
#------------------------------------------------------------------------------
# Captură trafic în background
#------------------------------------------------------------------------------
start_capture() {
    log_step "Pornire captură trafic..."
    
    # Verificare tshark/tcpdump
    if command -v tshark &>/dev/null; then
        log_info "Utilizare tshark pentru captură"
        ...
        CAPTURE_PID=$!
        log_info "Captură pornită (PID: $CAPTURE_PID)"
    elif command -v tcpdump &>/dev/null; then
        log_info "Utilizare tcpdump pentru captură"
        ...
        log_warn "tshark/tcpdump indisponibil - captură omisă"
        CAPTURE_PID=""
        # Crează fișier placeholder
        touch "$PCAP_FILE"
    fi
}
...
stop_capture() {
    if [ -n "${CAPTURE_PID:-}" ]; then
        log_info "Oprire captură (PID: $CAPTURE_PID)..."
...
test_dns() {
    ...
    if docker compose -f "$DOCKER_DIR/docker-compose.yml" exec -T debug dig +short web 2>>"$LOG_FILE"; then
        ...
        log_info "✓ DNS implicit funcționează"
    else
        result="dns_implicit:FAIL"
        log_error "✗ DNS implicit eșuat"
    fi
    ...
    if dns_response=$(docker compose -f "$DOCKER_DIR/docker-compose.yml" exec -T debug dig @dns-server -p $DNS_PORT myservice.lab.local +short 2>>"$LOG_FILE"); then
        if [ -n "$dns_response" ]; then
            ...
            log_info "✓ DNS custom răspunde: $dns_response"
        else
            result="dns_custom:PARTIAL"
            log_warn "⚠ DNS custom răspunde dar fără rezultat"
        fi
    else
        result="dns_custom:FAIL"
        log_error "✗ DNS custom eșuat"
    fi
    echo "$result" >> "$VALIDATION_FILE"
    
    # Test 3: dig de pe host (dacă disponibil)
...
test_ssh() {
    ...
    if banner=$(timeout 3 nc -v localhost $SSH_PORT 2>&1 | head -1); then
        if echo "$banner" | grep -qi "ssh"; then
            ...
            log_warn "⚠ Răspuns dar fără banner SSH standard"
        fi
    else
        result="ssh_banner:FAIL"
        log_error "✗ Nu s-a putut obține banner"
    fi
    echo "$result" >> "$VALIDATION_FILE"
    
    # Test 3: SSH cu Paramiko (din container)
...
" 2>>"$LOG_FILE" | tee -a "$LOG_FILE" | grep -q "SUCCESS"; then
        ...
        log_info "✓ Conexiune Paramiko reușită"
    else
        result="ssh_paramiko:FAIL"
        log_error "✗ Conexiune Paramiko eșuată"
    fi
    echo "$result" >> "$VALIDATION_FILE"
}
...
test_ftp() {
    ...
    if banner=$(timeout 3 nc -v localhost $FTP_PORT 2>&1 | head -1); then
        if echo "$banner" | grep -qi "220"; then
            ...
            log_warn "⚠ Răspuns dar fără cod 220"
        fi
    else
        result="ftp_banner:FAIL"
        log_error "✗ Nu s-a putut obține banner FTP"
    fi
    echo "$result" >> "$VALIDATION_FILE"
    
    # Test 3: FTP cu curl (din container)
    ...
    if docker compose -f "$DOCKER_DIR/docker-compose.yml" exec -T debug \
        ...
        log_info "✓ Listare FTP reușită"
    else
        result="ftp_list:FAIL"
        log_error "✗ Listare FTP eșuată"
    fi
    echo "$result" >> "$VALIDATION_FILE"
}
...
test_web() {
    ...
    if docker compose -f "$DOCKER_DIR/docker-compose.yml" exec -T debug \
        ...
        log_info "✓ HTTP via DNS implicit funcționează"
    else
        result="http_docker_dns:FAIL"
        log_error "✗ HTTP via DNS implicit eșuat"
    fi
    echo "$result" >> "$VALIDATION_FILE"
}

#------------------------------------------------------------------------------
# Generare sumar validare
#------------------------------------------------------------------------------
generate_validation_summary() {
    ...
    if [ $failed -gt 0 ]; then
        log_warn "Atenție: $failed teste eșuate"
        return 1
    else
...
cleanup_demo() {
    ...
    # Nu oprim containerele - le lăsăm pentru explorare
    log_info "Containerele rămân active pentru explorare"
...
main() {
    echo ""
    echo "╔═══════════════════════════════════════════════════════════════════════════════╗"
    echo "║  DEMO AUTOMAT - SĂPTĂMÂNA 10 - SERVICII DE REȚEA ÎN CONTAINERE                ║"
    echo "║  DNS | SSH | FTP | Docker Compose                                             ║"
    echo "║  Rețele de Calculatoare | ASE București | 2025-2026                           ║"
    echo "╚═══════════════════════════════════════════════════════════════════════════════╝"
    echo ""
    ...
    if ! check_prerequisites; then
        log_error "Prerequisite neîndeplinite. Ieșire."
        exit 1
    fi
    ...
    log_info "Captură PCAP: $PCAP_FILE"
    log_info "Validare:     $VALIDATION_FILE"
    echo ""
    
    # Afișare conținut validare
