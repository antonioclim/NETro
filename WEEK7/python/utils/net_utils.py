"""Utilitare pentru scripturile din Săptămâna 7.

Acest modul urmărește să păstreze dependențele minimale și comportamentul predictibil.
"""
...
def run_cmd(argv: Iterable[str], timeout: Optional[int] = None, check: bool = False, cwd: Optional[Path] = None) -> CmdResult:
    ...
    if check and res.returncode != 0:
        raise RuntimeError(
            f"Comanda a eșuat: {shlex.join(argv_list)}\nstdout:\n{res.stdout}\nstderr:\n{res.stderr}"
        )
    return res
