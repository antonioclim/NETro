...
"""
═══════════════════════════════════════════════════════════════════════════════
 Săptămâna 2 – Exercițiul 2: Server/Client UDP cu Protocol Aplicație
═══════════════════════════════════════════════════════════════════════════════

 OBIECTIVE:
 1. Socket-uri UDP (SOCK_DGRAM)
 2. Diferența TCP vs UDP (connectionless)
 3. Protocol aplicație peste UDP
 ...
 UTILIZARE:
   ...
   O comandă:  python3 ex_2_02_udp.py client --host 127.0.0.1 --port 9998 -o "ping"
═══════════════════════════════════════════════════════════════════════════════
"""
...
HELP_TEXT = """Comenzi: ping, upper:<text>, lower:<text>, reverse:<text>, time, help, exit"""
...
def process_command(data: bytes) -> bytes:
    """Procesează comanda conform protocolului."""
    try:
...
def run_server(cfg: ServerConfig) -> None:
    ...
    log("SERVER", f"UDP pe {cfg.bind}:{cfg.port}")
    log("SERVER", "Așteptare datagrame... (Ctrl+C oprire)")
    ...
    try:
        ...
        log("SERVER", f"Oprire | Total: {count}")
    finally:
        sock.close()
...
def run_interactive(host: str, port: int, timeout: float) -> None:
    ...
    log("CLIENT", "Comenzi: ping, upper:text, time, help, exit")
    print()
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="S2 – Server/Client UDP")
