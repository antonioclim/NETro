#!/usr/bin/env python3
"""Topologie S2 (bază): 1 switch, 3 host-uri.
...
În modul --test, scriptul:
- porneşte un server TCP pe h1, trimite mesaje din h2/h3 (inclusiv concurent);
- porneşte un server UDP pe h1, trimite mesaje din h2;
- creează o captură tcpdump pe h2 pentru traficul TCP.

Nota: Mininet necesită privilegii (root).
"""
...
def run_demo(net: Mininet) -> None:
    ...
    tcp_srv = h1.popen(["python3", "-u", ex_tcp, "server", "--bind", "10.0.0.1", "--port", "9999", "--mode", "threaded"])
    time.sleep(0.5)

    info("*** Capturing TCP traffic on h2 (tcpdump)\n")
    cap_path = cap_dir / "tcp_9999_h2.pcap"
    tcpdump = h2.popen(["tcpdump", "-i", "h2-eth0", "-w", str(cap_path), "tcp", "port", "9999"],
                       stdout=open(os.devnull, "wb"), stderr=open(os.devnull, "wb"))
    time.sleep(0.3)

    info("*** Sending concurrent TCP clients from h2 and h3\n")
    c2 = h2.popen(["python3", ex_tcp, "client", "--host", "10.0.0.1", "--port", "9999", "--message", "hello from h2"])
    c3 = h3.popen(["python3", ex_tcp, "client", "--host", "10.0.0.1", "--port", "9999", "--message", "hello from h3"])
    c2.wait(); c3.wait()
    ...
    udp_srv = h1.popen(["python3", "-u", ex_udp, "server", "--bind", "10.0.0.1", "--port", "9998"])
    time.sleep(0.5)

    info("*** UDP client (once) from h2\n")
    h2.cmd(["python3", ex_udp, "client", "--host", "10.0.0.1", "--port", "9998", "--once", "ping"])
