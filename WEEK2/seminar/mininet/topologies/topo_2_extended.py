#!/usr/bin/env python3
"""Topologie S2 (extinsă): 2 subreţele + router (L3).

Idee didactică:
- L2: switch-urile conectează host-uri în aceeaşi reţea (acelaşi prefix).
- L3: router-ul conectează reţele diferite şi decide rutarea pe baza tabelei de rutare.

Topologie:
  (h1,h2) -- s1 -- r1 -- s2 -- (h3,h4)

IP plan:
  h1: 10.0.1.1/24   gw 10.0.1.254
  h2: 10.0.1.2/24   gw 10.0.1.254
  h3: 10.0.2.3/24   gw 10.0.2.254
  h4: 10.0.2.4/24   gw 10.0.2.254
...
class LinuxRouter(Node):
    """Node cu IP forwarding activ."""

    def config(self, **params):
        super().config(**params)
        self.cmd("sysctl -w net.ipv4.ip_forward=1")
        # rp_filter poate bloca pachete în anumite scenarii de test; îl dezactivăm pentru laborator.
...
class S2ExtendedTopo(Topo):
    def build(self, bw: float = 10.0, delay: str = "5ms", loss: float = 0.0) -> None:
        ...
        h3 = self.addHost("h3", ip="10.0.2.3/24", defaultRoute="via 10.0.2.254")
        h4 = self.addHost("h4", ip="10.0.2.4/24", defaultRoute="via 10.0.2.254")
...
def configure_router(net: Mininet) -> None:
    r1 = net.get("r1")
    # Setăm adresa pe interfaţa către s2 (eth1). eth0 a primit ip din params (10.0.1.254/24).
    r1.cmd("ip addr add 10.0.2.254/24 dev r1-eth1")
    r1.cmd("ip link set r1-eth0 up")
    r1.cmd("ip link set r1-eth1 up")
    info("*** Router configured: r1-eth0=10.0.1.254/24, r1-eth1=10.0.2.254/24\n")
...
def main() -> None:
    ...
    ap.add_argument("--test", action="store_true", help="rulează un auto-test (ping + rute) şi opreşte")
    args = ap.parse_args()
    ...
    try:
        ...
        if args.test:
            ...
            net.pingAll()
            info("*** Auto-test: ping h1 -> h3 (traversează r1)\n")
            net.get('h1').cmd('ping -c 2 10.0.2.3')
            info("*** Auto-test finalizat.\n")
        elif args.cli:
            CLI(net)
        else:
            info("*** Rulare fără CLI. Folosiţi --cli pentru explorare interactivă sau --test pentru auto-test.\n")
    finally:
        net.stop()
