#!/bin/bash
# ============================================================================
# cleanup.sh - Script de curățare pentru laboratorul S13
# ============================================================================
# Curăță toate resursele create în timpul laboratorului:
# - Containere Docker
# - Procese Mosquitto
# - Sesiuni Mininet
# - Fișiere temporare
...
cleanup_docker() {
    ...
    # Oprire containere din docker-compose
    if [ -f "$PROJECT_DIR/docker-compose.yml" ]; then
        cd "$PROJECT_DIR"
        docker-compose down -v 2>/dev/null || true
    fi
    
    # Oprire containere individuale (dacă există)
    ...
    # Curățare imagini nefolosite (opțional cu --deep)
    if [ "$DEEP_CLEAN" = true ]; then
        log_info "  Removing unused Docker images..."
        docker image prune -f 2>/dev/null || true
    fi
    
    # Curățare rețele Docker orfane
...
cleanup_mininet() {
    ...
    # Verificare dacă Mininet este instalat
...
cleanup_processes() {
    ...
    # Așteptare pentru terminare grațioasă
    sleep 1
    
    # Force kill dacă încă există
    for proc in "${processes[@]}"; do
        local pids=$(pgrep -f "$proc" 2>/dev/null || true)
        if [ -n "$pids" ]; then
...
cleanup_temp_files() {
    ...
    # Fișiere temporare în project dir
...
cleanup_certificates() {
    ...
    log_info "Certificates cleanup complete"
}
...
verify_cleanup() {
    ...
    if [ "$issues" -eq 0 ]; then
        log_info "  ✓ All resources cleaned successfully"
    else
