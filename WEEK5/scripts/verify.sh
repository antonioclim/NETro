#!/usr/bin/env bash
#===============================================================================
# verify.sh — Verificare completă a mediului și demo-urilor
#===============================================================================
# Utilizare: ./scripts/verify.sh [--quick|--full|--demos]
#===============================================================================
...
verify_system() {
    ...
    if command -v mn &> /dev/null; then
        log_pass "Mininet disponibil"
    else
        log_warn "Mininet nu este disponibil (necesită VM Linux)"
    fi
    ...
    for module in ipaddress argparse json dataclasses math; do
        if python3 -c "import $module" 2>/dev/null; then
            log_pass "Modul Python '$module' disponibil"
        else
            log_fail "Modul Python '$module' lipsește"
        fi
    done
}

#-------------------------------------------------------------------------------
# Verificări structură proiect
#-------------------------------------------------------------------------------

verify_structure() {
    log_section "Verificări structură proiect"
    ...
    for dir in "${required_dirs[@]}"; do
        if [[ -d "$PROJECT_ROOT/$dir" ]]; then
            ...
            log_warn "Director lipsă: $dir"
        fi
    done
}

#-------------------------------------------------------------------------------
# Verificări demo-uri Python
#-------------------------------------------------------------------------------

verify_python_demos() {
    log_section "Verificări demo-uri Python"
    ...
    if python3 ex_5_02_vlsm_ipv6.py ipv6 2001:db8::1 > /dev/null 2>&1; then
        log_pass "IPv6 compress funcțional"
    else
        log_fail "IPv6 compress eșuat"
    fi
    ...
    if python3 -c "import ex_5_03_quiz_generator" 2>/dev/null; then
        ...
        log_fail "Quiz generator are erori de sintaxă"
    fi
...
#-------------------------------------------------------------------------------
# Verificări Mininet (dacă disponibil)
#-------------------------------------------------------------------------------

verify_mininet() {
    log_section "Verificări Mininet"
    
    if ! command -v mn &> /dev/null; then
        log_warn "Mininet nu este disponibil, skip verificări"
        return
    fi
    
    # Verificare sintaxă topologii
    log_info "Verificare sintaxă topologii..."
    
    if python3 -m py_compile "$PROJECT_ROOT/mininet/topologies/topo_5_base.py" 2>/dev/null; then
        log_pass "topo_5_base.py sintaxă validă"
    else
        log_fail "topo_5_base.py erori sintaxă"
    fi
    
    if python3 -m py_compile "$PROJECT_ROOT/mininet/topologies/topo_5_extended.py" 2>/dev/null; then
        log_pass "topo_5_extended.py sintaxă validă"
    else
        log_fail "topo_5_extended.py erori sintaxă"
    fi
    
    # Test funcțional (necesită sudo)
    if sudo -n true 2>/dev/null; then
        log_info "Testare topologie de bază (necesită ~10 secunde)..."
        
        cd "$PROJECT_ROOT/mininet/topologies"
        if sudo timeout 30 python3 topo_5_base.py --test > /dev/null 2>&1; then
            log_pass "topo_5_base.py test conectivitate reușit"
        else
            log_warn "topo_5_base.py test conectivitate eșuat sau timeout"
        fi
        cd - > /dev/null
        ...
        log_warn "Sudo necesită parolă, skip test Mininet funcțional"
    fi
}
...
print_summary() {
    ...
    if [[ $FAIL -eq 0 ]]; then
        echo -e "${GREEN}✓ Mediul este pregătit pentru Săptămâna 5!${NC}"
        return 0
    else
        echo -e "${RED}✗ $FAIL verificări eșuate. Consultați README.md pentru remediere.${NC}"
        return 1
    fi
}
...
main() {
    echo ""
    echo "╔══════════════════════════════════════════════════════════════╗"
    echo "║            Verificare Mediu — Săptămâna 5                    ║"
    echo "╚══════════════════════════════════════════════════════════════╝"
    ...
    case "$mode" in
        ...
        --demos)
            ...
            ;;
        --help|-h)
            echo "Utilizare: $0 [--quick|--full|--demos]"
            echo ""
            echo "Opțiuni:"
            echo "  --quick  Doar verificări sistem"
            echo "  --full   Verificare completă (implicit)"
            echo "  --demos  Doar demo-uri Python și Mininet"
            exit 0
            ;;
        *)
