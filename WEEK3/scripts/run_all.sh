#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════════════
# run_all.sh — Demo automat complet pentru Săptămâna 3
# ═══════════════════════════════════════════════════════════════════════════════
#
# WEEK 3: Broadcast & Multicast UDP + TCP Tunnel
#
# DESCRIERE:
#   Rulează automat toate demonstrațiile săptămânii fără input interactiv.
#   Produce artefacte: demo.log, demo.pcap, validation.txt în artifacts/
#
# UTILIZARE:
#   sudo bash scripts/run_all.sh           # Demo complet
#   sudo bash scripts/run_all.sh --quick   # Demo rapid (doar localhost)
#   sudo bash scripts/run_all.sh --mininet # Demo cu topologie Mininet
#
# OUTPUT:
#   artifacts/demo.log         - Log complet al tuturor demo-urilor
#   artifacts/demo.pcap        - Captură trafic (broadcast + multicast)
#   artifacts/validation.txt   - Rezultate validare
#
# LICENȚĂ: MIT | Hypotheticalandrei & Rezolvix | CSIE/ASE 2025
# ═══════════════════════════════════════════════════════════════════════════════
...
# ─── Configurare WEEK 3 ──────────────────────────────────────────────────────
WEEK=3
WEEK_PORT_BASE=$((5100 + 100 * (WEEK - 1)))  # 5300 pentru WEEK 3
...
# ─── Verificare cerințe ──────────────────────────────────────────────────────
check_requirements() {
    log "INFO" "Verificare cerințe sistem..."
    ...
    if [ ${#missing[@]} -gt 0 ]; then
        log "ERROR" "Lipsesc: ${missing[*]}"
        return 1
    fi
    
    log "INFO" "Toate cerințele sunt îndeplinite."
    return 0
}
...
demo_broadcast_localhost() {
    ...
    log "INFO" "Pornesc sender către 127.255.255.255:$UDP_BCAST_PORT..."
...
demo_multicast_localhost() {
    ...
    log "INFO" "Pornesc sender multicast către $MULTICAST_GROUP:$UDP_MCAST_PORT..."
...
demo_tunnel_localhost() {
    ...
    if [[ "$response" == *"HELLO"* ]] || [[ "$response" == *"TUNNEL"* ]]; then
        log "INFO" "Tunnel funcționează: răspuns primit"
        echo "TCP_TUNNEL_LOCALHOST=PASS" >> "$ARTIFACTS_DIR/validation.txt"
    else
        log "WARN" "Tunnel: răspuns incomplet (primit: '$response')"
...
# ─── Demo 4: Captură Trafic ──────────────────────────────────────────────────
demo_capture_traffic() {
    log_section "DEMO 4: Captură Trafic (tcpdump)"
    
    local pcap_file="$ARTIFACTS_DIR/demo.pcap"
    
    log "INFO" "Pornesc captură pe loopback pentru 10 secunde..."
    ...
    # Generăm trafic
    log "INFO" "Generez trafic pentru captură..."
...
demo_multiclient() {
    ...
    log "INFO" "Trimit 3 conexiuni simultane..."
...
generate_summary() {
    ...
    if $artifacts_ok; then
        ...
        log "INFO" "${GREEN}DEMO COMPLET - Toate artefactele generate${NC}"
    else
        echo "OVERALL_STATUS=PARTIAL" >> "$ARTIFACTS_DIR/validation.txt"
        log "WARN" "${YELLOW}DEMO PARȚIAL - Unele artefacte lipsesc${NC}"
    fi
...
main() {
    ...
    echo ""
    echo "╔══════════════════════════════════════════════════════════════════════╗"
    echo "║  WEEK $WEEK: Broadcast & Multicast UDP + TCP Tunnel — Demo Automat     ║"
    echo "╚══════════════════════════════════════════════════════════════════════╝"
    echo ""
    
    log "INFO" "Inițializare demo WEEK $WEEK (mod: $mode)..."
    ...
    if ! check_requirements; then
        log "ERROR" "Cerințe neîndeplinite. Abandonez."
        exit 1
    fi
    ...
    case "$mode" in
        --quick|-q)
            ...
            ;;
        --mininet|-m)
            log "WARN" "Mod Mininet necesită rulare separată cu sudo. Folosind localhost."
            demo_broadcast_localhost
            demo_multicast_localhost
            demo_tunnel_localhost
            demo_multiclient
