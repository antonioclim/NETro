...
check() {
    if command -v "$1" &> /dev/null; then
        ...
        echo -e "${RED}[FAIL]${NC} $1 nu este instalat"
        ((ERRORS++))
    fi
}

echo -e "${CYAN}[VERIFY] Verificare dependențe...${NC}"
check python3
check tcpdump
check nc
...
if sudo mn --test pingall &>/dev/null; then
    echo -e "${GREEN}[OK]${NC} Mininet funcționează"
else
    echo -e "${RED}[WARN]${NC} Mininet poate necesita configurare"
fi

if [ $ERRORS -eq 0 ]; then
    echo -e "${GREEN}[OK] Toate verificările au trecut!${NC}"
    exit 0
else
