#!/usr/bin/env python3
"""
ex_01_smtp.py - Exerciții SMTP Self-Contained pentru Săptămâna 12

Acest fișier conține un server SMTP minimal și client pentru demonstrarea
conceptelor de email fără dependențe externe.

CE VOM ÎNVĂȚA:
- Protocolul SMTP și comenzile sale fundamentale
- Diferența între envelope (MAIL FROM, RCPT TO) și message headers (From:, To:)
- Stocarea și parsarea mesajelor .eml
- Structura MIME pentru atașamente

DE CE CONTEAZĂ:
- Email-ul rămâne principalul canal de comunicare business
- Înțelegerea SMTP e esențială pentru debugging-ul problemelor de email
- Conceptele de envelope vs message se regăsesc în multe protocoale
...
# =============================================================================
# CONCEPTE TEORETICE (în comentarii pentru studiu)
# =============================================================================
"""
...
ARHITECTURA EMAIL:
    ...
    Client de email → Server SMTP → Server destinatar → Mailbox
...
COMENZI SMTP:
    HELO/EHLO  - Salut inițial
    MAIL FROM: - Specifică expeditorul (envelope)
    RCPT TO:   - Specifică destinatarul (envelope)
    DATA       - Începe transferul mesajului
    QUIT       - Închide conexiunea
    RSET       - Resetează tranzacția
    NOOP       - Keep-alive

CODURI RĂSPUNS:
    2xx - Succes
    3xx - Continuare (ex: 354 pentru DATA)
...
class MiniSMTPServer:
    """
    Server SMTP minimal pentru scopuri educaționale.
    
    Implementează subset de comenzi pentru a demonstra protocolul.
    """
    ...
    def start(self):
        ...
        print("[SMTP] Apăsați Ctrl+C pentru a opri\n")
    ...
    def _save_message(self, mail_from: str, rcpt_to: list, data: str):
        ...
        print(f"[SMTP] Mesaj salvat: {filepath}")


def send_email(host: str, port: int, sender: str, recipient: str, 
               subject: str, body: str, verbose: bool = False):
    """
    Trimite un email folosind smtplib.
    
    Demonstrează utilizarea bibliotecii standard Python.
    """
...
def list_messages(spool_dir: str):
    """Listează mesajele din spool."""
    spool = Path(spool_dir)
    if not spool.exists():
        print(f"[INFO] Spool '{spool_dir}' nu există.")
        return
    
    files = list(spool.glob("*.eml"))
    if not files:
        print(f"[INFO] Niciun mesaj în {spool_dir}")
        return
    
    print(f"\n{'='*60}")
    print(f"Mesaje în {spool.absolute()} ({len(files)} total)")
    print(f"{'='*60}\n")
    ...
    for filepath in sorted(files):
        ...
        subject = subject_match.group(1) if subject_match else "(fără subiect)"
...
def analyze_envelope_vs_headers(spool_dir: str):
    """
    Analizează diferența între envelope și message headers.
    
    Aceasta este o lecție cheie despre SMTP!
    """
    spool = Path(spool_dir)
    files = list(spool.glob("*.eml")) if spool.exists() else []
    
    if not files:
        print("[INFO] Niciun mesaj de analizat. Trimiteți câteva emailuri mai întâi!")
        return
    
    print("\n" + "="*70)
    print("ANALIZĂ: ENVELOPE vs MESSAGE HEADERS")
    print("="*70)
...
║    - Folosit pentru RUTARE și LIVRARE                               ║
║    - Nu este vizibil în clientul de email al utilizatorului         ║
║                                                                      ║
║  MESSAGE HEADERS (scrisoarea):                                       ║
║    - From:, To:, Subject: din corpul mesajului RFC 5322             ║
║    - Afișate utilizatorului în clientul de email                    ║
║    - POT DIFERI de envelope!                                        ║
║                                                                      ║
║  De ce contează:                                                     ║
║    - Spoofing: From: poate fi falsificat ușor                       ║
        ...
        if env_from and hdr_from:
            ...
            if env_addr and hdr_addr:
                if env_addr.group() == hdr_addr.group():
                    ...
                    print("  ⚠ DIFERENȚĂ: Envelope From ≠ Message From!")
...
def run_selftest():
    ...
    # Test 4: Verificare salvare
    print("\n[TEST 4] Verificare mesaj salvat...")
    time.sleep(0.3)
    files = list(Path(spool_dir).glob("*.eml"))
    if files:
        print(f"  ✓ Mesaj găsit: {files[0].name}")
    else:
        print("  ✗ Niciun mesaj salvat")
        return False
...
def main():
    parser = argparse.ArgumentParser(
        description="Exerciții SMTP Self-Contained - Săptămâna 12",
...
Revolvix&Hypotheticalandrei
    ...
    # Server
    p_server = subparsers.add_parser('server', help='Pornește serverul SMTP')
    p_server.add_argument('--port', type=int, default=1025)
    p_server.add_argument('--spool', default='./spool_s12')
    
    # Send
    p_send = subparsers.add_parser('send', help='Trimite un email')
    ...
    p_send.add_argument('--body', default='Mesaj de test din exercițiul SMTP.')
    p_send.add_argument('-v', '--verbose', action='store_true')
    
    # List
    p_list = subparsers.add_parser('list', help='Listează mesajele')
    p_list.add_argument('--spool', default='./spool_s12')
    
    # Analyze
    p_analyze = subparsers.add_parser('analyze', help='Analizează envelope vs headers')
    p_analyze.add_argument('--spool', default='./spool_s12')
    
    # Selftest
    parser.add_argument('--selftest', action='store_true', help='Rulează auto-teste')
