#!/usr/bin/env bash
# =============================================================================
# benchmark_rpc.sh — Săptămâna 12: Benchmark JSON-RPC vs XML-RPC
# =============================================================================
# Compară performanța celor două protocoale RPC
# Utilizare: ./scripts/benchmark_rpc.sh [--iterations N] [--warmup N]
# =============================================================================
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --help|-h)
            ...
            echo "  --iterations, -n  Number of test iterations (default: 100)"
            echo "  --warmup, -w      Number of warmup iterations (default: 10)"
            exit 0
            ;;
        *) shift ;;
    esac
...
echo -e "${CYAN}═══════════════════════════════════════════════════════════════${NC}"
echo -e "${CYAN}       BENCHMARK: JSON-RPC vs XML-RPC Performance${NC}"
echo -e "${CYAN}═══════════════════════════════════════════════════════════════${NC}"
echo ""
...
wait_for_port "$JSONRPC_PORT" && log_success "JSON-RPC server ready"
wait_for_port "$XMLRPC_PORT" && log_success "XML-RPC server ready"
sleep 1
...
for i in $(seq 1 $WARMUP); do
    curl -s -X POST -H "Content-Type: application/json" \
        ...
        http://localhost:$JSONRPC_PORT/ > /dev/null
    ...
    curl -s -X POST -H "Content-Type: text/xml" \
        ...
        http://localhost:$XMLRPC_PORT/ > /dev/null
done
...
echo ""
log_result "JSON-RPC: Total ${JSONRPC_TIME}s, Avg ${JSONRPC_AVG}ms per request"
log_result "XML-RPC:  Total ${XMLRPC_TIME}s, Avg ${XMLRPC_AVG}ms per request"

if (( $(echo "$JSONRPC_TIME < $XMLRPC_TIME" | bc -l) )); then
    SPEEDUP=$(echo "scale=2; $XMLRPC_TIME / $JSONRPC_TIME" | bc)
    log_success "JSON-RPC is ${SPEEDUP}x faster"
else
    SPEEDUP=$(echo "scale=2; $JSONRPC_TIME / $XMLRPC_TIME" | bc)
    log_success "XML-RPC is ${SPEEDUP}x faster"
fi
echo ""

# =============================================================================
# BENCHMARK 2: Multiple Operations (Batch-like)
# =============================================================================
echo -e "${YELLOW}═══ TEST 2: Multiple Sequential Operations ═══${NC}"
...
echo ""
log_result "JSON-RPC: ${JSONRPC_MULTI}s for $((BATCH_ITERATIONS * 3)) operations"
log_result "XML-RPC:  ${XMLRPC_MULTI}s for $((BATCH_ITERATIONS * 3)) operations"
echo ""
...
JSONRPC_RESP=$(curl -s -X POST -H "Content-Type: application/json" -d "$JSONRPC_REQ" http://localhost:$JSONRPC_PORT/)
...
XMLRPC_RESP=$(curl -s -X POST -H "Content-Type: text/xml" -d "$XMLRPC_REQ" http://localhost:$XMLRPC_PORT/)
...
echo "  Difference: $((XMLRPC_REQ_SIZE - JSONRPC_REQ_SIZE)) bytes (XML larger)"
echo ""
echo -e "${BLUE}Response Sizes:${NC}"
echo "  JSON-RPC: $JSONRPC_RESP_SIZE bytes"
echo "  XML-RPC:  $XMLRPC_RESP_SIZE bytes"
echo "  Difference: $((XMLRPC_RESP_SIZE - JSONRPC_RESP_SIZE)) bytes (XML larger)"
echo ""
...
echo "  • XML-RPC has built-in introspection capabilities"
echo "  • For high-throughput systems, consider gRPC (binary protocol)"
echo "  • Choice depends on requirements: interoperability, debugging, performance"
