#!/usr/bin/env python3
"""
smtp_server.py - Server SMTP Didactic pentru Săptămâna 12

Acest modul implementează un server SMTP simplificat conform RFC 5321,
demonstrând conceptele fundamentale ale protocolului de transfer email.
...
Scopul educațional:
    - Înțelegerea comenzilor SMTP (HELO, MAIL FROM, RCPT TO, DATA, QUIT)
    - Diferența între envelope (plic) și message (mesaj)
    - Coduri de răspuns SMTP (2xx, 3xx, 4xx, 5xx)
    - Stocarea mesajelor în format .eml
...
class SMTPState(Enum):
    """Stările protocolului SMTP conform RFC 5321."""
    INIT = auto()           # Conexiune nouă, așteptăm HELO/EHLO
    GREETED = auto()        # HELO acceptat
    MAIL_FROM = auto()      # MAIL FROM acceptat, așteptăm RCPT TO
    RCPT_TO = auto()        # Cel puțin un RCPT TO acceptat
    DATA = auto()           # În modul DATA, primim corpul mesajului
    QUIT = auto()           # Sesiune încheiată
...
class SMTPEnvelope:
    ...
    Conceptul de "plic" vs "mesaj":
    - Envelope (plicul): informații de rutare (MAIL FROM, RCPT TO)
    - Message (mesajul): antetele RFC 5322 + corpul mesajului
    
    Analogie: scrisoarea într-un plic poștal
    - Pe plic: adresa expeditorului, adresa destinatarului
    - În plic: scrisoarea propriu-zisă (poate avea alt "From" în antet!)
    """
    mail_from: str = ""
    ...
    def reset(self):
        """Resetează envelope-ul pentru o nouă tranzacție."""
        self.mail_from = ""
        self.rcpt_to = []
        self.data = ""
...
class SMTPHandler:
    """
    Handler pentru o conexiune SMTP individuală.
    ...
    def handle(self):
        """Gestionează conexiunea SMTP."""
        try:
            # Banner inițial
            self._send(f"{self.CODES['READY']} {self.hostname} SMTP Ready")
            logger.info(f"[{self.addr[0]}:{self.addr[1]}] Conexiune nouă")
            ...
            while True:
                ...
                # Dispatch către handler-ul comenzii
            ...
            logger.error(f"[{self.addr[0]}] Eroare: {e}")
        finally:
            self.socket.close()
            logger.info(f"[{self.addr[0]}:{self.addr[1]}] Conexiune închisă")
    ...
    def _recv_data(self) -> str:
        """Primește corpul mesajului până la <CRLF>.<CRLF>."""
        data_lines = []
        while True:
            line = self._recv()
            if line is None:
            ...
            if line == ".":
                break
            # Dot-stuffing: linia care începe cu "." are "." eliminat
            if line.startswith(".."):
                line = line[1:]
    ...
    def _cmd_helo(self, argument: str) -> str:
        """
        HELO - Salut simplu (SMTP clasic).
        
        Format: HELO <hostname>
        Răspuns: 250 <server-hostname>
        """
        if not argument:
            return f"{self.CODES['PARAM_ERROR']} Missing hostname"
    ...
    def _cmd_ehlo(self, argument: str) -> str:
        ...
        # Răspuns multi-linie cu extensiile suportate
        response_lines = [
    ...
    def _cmd_mail(self, argument: str) -> str:
        """
        MAIL FROM - Specifică expeditorul (envelope).
    ...
    def _cmd_rcpt(self, argument: str) -> str:
        """
        RCPT TO - Specifică un destinatar (envelope).
        
        Format: RCPT TO:<address>
        Răspuns: 250 OK
        
        Notă: Poate fi apelat de mai multe ori pentru mai mulți destinatari.
        """
    ...
    def _cmd_data(self, argument: str) -> str:
        """
        DATA - Inițiază transferul corpului mesajului.
        
        Format: DATA
        Răspuns: 354 Start mail input
        
        După răspunsul 354, clientul trimite:
        - Antetele RFC 5322 (From, To, Subject, Date, etc.)
        - O linie goală
        - Corpul mesajului
        - <CRLF>.<CRLF> pentru terminare
        """
        ...
        self.state = SMTPState.GREETED  # Gata pentru o nouă tranzacție
        logger.info(f"[{self.addr[0]}] Mesaj primit și salvat")
        
        return f"{self.CODES['OK']} Message accepted for delivery"
    
    def _cmd_rset(self, argument: str) -> str:
        """
        RSET - Resetează tranzacția curentă.
    ...
    def _cmd_quit(self, argument: str) -> str:
        ...
        return f"{self.CODES['BYE']} {self.hostname} closing connection"
    
    def _cmd_help(self, argument: str) -> str:
        """
        HELP - Informații despre comenzile suportate.
...
214 For more info: RFC 5321"""
    ...
    def _cmd_vrfy(self, argument: str) -> str:
        """VRFY - Verificare adresă (dezactivat din motive de securitate)."""
        return f"{self.CODES['CMD_NOT_IMPL']} VRFY disabled for security"
    
    def _cmd_expn(self, argument: str) -> str:
        """EXPN - Expandare listă (dezactivat din motive de securitate)."""
    ...
    def _save_message(self):
        """
        Salvează mesajul în format .eml.
        
        Structura fișierului .eml:
        1. Antet Received adăugat de server
        2. Antetele originale ale mesajului
        3. Linie goală
        4. Corpul mesajului
        """
        # Generare ID unic pentru mesaj
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            ...
            f.write("\r\n")  # Separator între envelope info și mesaj original
...
class SMTPServer:
    ...
    def start(self):
        ...
        self.running = True
        logger.info(f"Server SMTP pornit pe {self.listen_addr}:{self.port}")
...
def run_selftest():
    """
    Rulează auto-teste pentru validarea funcționalității.
    """
    import time
    ...
    try:
        ...
        print("  ✓ Server inițializat")
    except Exception as e:
        print(f"  ✗ Eroare: {e}")
        return False
    
    # Test 2: Pornire server în thread
    print("\n[TEST 2] Pornire server în background...")
    import threading
    server_thread = threading.Thread(target=server.start, daemon=True)
    server_thread.start()
    time.sleep(0.5)  # Așteptăm pornirea
...
def main():
    parser = argparse.ArgumentParser(
        description="Server SMTP Didactic pentru Săptămâna 12",
