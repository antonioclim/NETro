#!/usr/bin/env python3
"""
jsonrpc_server.py - Server JSON-RPC 2.0 Didactic pentru Săptămâna 12

Acest modul implementează un server JSON-RPC 2.0 conform specificației oficiale:
https://www.jsonrpc.org/specification

Concepte demonstrate:
- Structura cererii JSON-RPC (jsonrpc, method, params, id)
- Structura răspunsului (jsonrpc, result/error, id)
...
class JSONRPCError:
    ...
    def make_error(code: int, message: str, data: Any = None) -> Dict:
        """Construiește un obiect de eroare JSON-RPC."""
        error = {
            "code": code,
            "message": message
        }
...
class RPCMethods:
    """
    Clasa cu metodele expuse prin JSON-RPC.
    
    Fiecare metodă publică (fără _) devine disponibilă pentru apel remote.
    """
    ...
    def divide(a: float, b: float) -> float:
        ...
        Raises:
            ValueError: Dacă b este 0 (împărțire la zero)
        """
        if b == 0:
    ...
    def echo(message: str) -> str:
        """Returnează mesajul primit (util pentru testing)."""
        return message
    ...
    def get_server_info() -> Dict[str, str]:
        """
        Returnează informații despre server.
        
        Returns:
            Dict cu: name, version, protocol
        """
        return {
    ...
    def sort_list(items: List[Any], reverse: bool = False) -> List[Any]:
        ...
        Args:
            items: Lista de sortat
            reverse: True pentru sortare descrescătoare
        
        Returns:
            Lista sortată
        
        Exemplu cu parametri poziționali:
            {"method":"sort_list","params":[[3,1,2],false],"id":1}
        
        Exemplu cu parametri numiți:
    ...
    def benchmark(iterations: int = 1000) -> Dict[str, Any]:
        """
        Rulează un micro-benchmark pentru măsurarea overhead-ului.
...
# =============================================================================
# Handler HTTP pentru JSON-RPC
# =============================================================================

class JSONRPCHandler(BaseHTTPRequestHandler):
    """
    Handler HTTP care procesează cereri JSON-RPC 2.0.
    """
    
    # Referință către obiectul cu metode RPC
    ...
    def do_POST(self):
        """Procesează cereri POST (JSON-RPC)."""
        # Citire conținut
        ...
        if isinstance(request, list):
            ...
            for req in request:
                ...
                if response is not None:  # Nu includem răspunsuri pentru notificări
                    responses.append(response)
            
            if responses:
                self._send_response(responses)
            else:
                # Toate au fost notificări
            ...
            if response is not None:
                ...
                # Notificare - nu trimitem răspuns
                self.send_response(204)
                self.end_headers()
    
    def do_GET(self):
        """Răspunde la GET cu informații despre server."""
        info = {
    ...
    def _process_request(self, request: Dict) -> Optional[Dict]:
        """
        Procesează o cerere JSON-RPC individuală.
        ...
        Structura cererii JSON-RPC 2.0:
        {
            ...
            "params": [...] sau {...},   # Opțional
            "id": 1                       # Opțional (lipsa => notificare)
        }
        
        Returns:
            Dict cu răspunsul sau None pentru notificări
        """
        ...
        req_id = request.get("id")  # Poate fi None pentru notificări
        
        # jsonrpc trebuie să fie "2.0"
        if jsonrpc != "2.0":
            return self._make_error_response(
                JSONRPCError.INVALID_REQUEST,
                'jsonrpc must be "2.0"',
        ...
        # params trebuie să fie array sau object (dacă e prezent)
        ...
        try:
            ...
            # Dacă e notificare (fără id), nu returnăm răspuns
            if req_id is None:
                return None
    ...
    def _make_error_response(self, code: int, message: str, req_id: Any) -> Dict:
        """Construiește un răspuns de eroare JSON-RPC."""
        return {
            "jsonrpc": "2.0",
    ...
    def _send_error(self, code: int, message: str):
        """Trimite o eroare la nivel HTTP."""
...
def run_server(listen: str = "0.0.0.0", port: int = 8080):
    ...
    logger.info("Apăsați Ctrl+C pentru a opri")
...
def run_selftest():
    ...
    if "name" in response.get("result", {}):
        ...
        print(f"  ✗ Răspuns invalid: {response}")
        return False
    
    # Test 5: Metodă inexistentă
    print("\n[TEST 5] Apel metodă inexistentă...")
    response = call_rpc("nonexistent_method")
    if "error" in response and response["error"]["code"] == JSONRPCError.METHOD_NOT_FOUND:
        print(f"  ✓ Eroare corectă: {response['error']['message']}")
    else:
        print(f"  ✗ Eroare incorectă: {response}")
        return False
    
    # Test 6: Parametri numiți
    print("\n[TEST 6] Apel cu parametri numiți sort_list(items=[3,1,2], reverse=True)...")
...
def main():
    parser = argparse.ArgumentParser(
        description="Server JSON-RPC 2.0 Didactic pentru Săptămâna 12",
