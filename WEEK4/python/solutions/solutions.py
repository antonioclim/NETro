#!/usr/bin/env python3
"""
Soluții pentru Exercițiile Săptămânii 4
========================================

ATENȚIE: Acest fișier conține soluțiile complete.
Consultați-l doar după ce ați încercat să rezolvați exercițiile singuri!

Autor: Revolvix&Hypotheticalandrei
"""

# =============================================================================
# SOLUȚIE EXERCIȚIUL 4.01: Protocol TCP Custom
# =============================================================================
...
def sol_parse_command(data: str) -> tuple:
    """
    SOLUȚIE: Parsează mesajul primit și extrage comanda, lungimea și payload-ul.
    """
    parts = data.split(' ', 2)
    ...
    # Verificăm consistența lungimii (în bytes UTF-8)
...
def sol_execute_command(command: str, payload: str) -> str:
    """
    SOLUȚIE: Execută comanda specificată pe payload.
    """
    if command == 'ECHO':
        return payload
    elif command == 'UPPER':
...
def sol_format_response(result: str) -> str:
    """
    SOLUȚIE: Formatează răspunsul pentru client.
    """
...
def sol_handle_client(client_socket: socket.socket, address: tuple):
    """
    SOLUȚIE: Gestionează comunicarea cu un client.
    """
...
def sol_start_tcp_server(host: str = '0.0.0.0', port: int = TCP_DEFAULT_PORT):
    """
    SOLUȚIE: Pornește serverul TCP multi-threaded.
    """
...
# =============================================================================
# SOLUȚIE EXERCIȚIUL 4.02: Agregator UDP Senzori
# =============================================================================
...
def sol_parse_sensor_datagram(datagram: bytes) -> Optional[Tuple[int, float, str]]:
    """
    SOLUȚIE: Parsează o datagramă primită de la un senzor.
    """
...
def sol_update_statistics(stats: Dict[int, SensorStatsSolution],
                          ...
                          location: str) -> None:
    """
    SOLUȚIE: Actualizează statisticile pentru un senzor.
    """
...
def sol_generate_report(stats: Dict[int, SensorStatsSolution]) -> dict:
    """
    SOLUȚIE: Generează un raport JSON cu toate statisticile.
    """
...
def sol_run_aggregator(host: str = '0.0.0.0',
    ...
    """
    SOLUȚIE: Rulează serverul agregator UDP.
    """
...
# =============================================================================
# FUNCȚII AJUTĂTOARE PENTRU TESTARE
# =============================================================================
...
if __name__ == '__main__':
    ...
    print("="*60)
    print("FIȘIER SOLUȚII - Exercițiile Săptămânii 4")
    print("="*60)
    print("""
Acest fișier conține soluțiile complete pentru:

1. sol_parse_command() - Parsare comandă protocol TCP
2. sol_execute_command() - Execuție comandă
3. sol_format_response() - Formatare răspuns
4. sol_handle_client() - Handler client TCP
5. sol_start_tcp_server() - Server TCP multi-threaded

6. sol_parse_sensor_datagram() - Parsare datagramă UDP
...
Utilizare:
  python3 solutions.py tcp     # Pornește serverul TCP (ex 4.01)
  python3 solutions.py udp     # Pornește agregatorul UDP (ex 4.02)
""")
    ...
    if len(sys.argv) > 1:
        ...
        print("Specificați 'tcp' sau 'udp' pentru a rula serverul corespunzător.")
