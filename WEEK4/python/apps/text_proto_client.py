#!/usr/bin/env python3
"""
Client TCP pentru protocolul text (length-prefixed framing).
...
def interactive_mode(conn: socket.socket) -> None:
    """Mod interactiv: citește comenzi de la stdin."""
...
def command_mode(conn: socket.socket, commands: list[str], verbose: bool) -> int:
    """Mod batch: execută lista de comenzi și afișează răspunsurile."""
    errors = 0
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Client TCP pentru protocolul text",
