#!/usr/bin/env python3
"""
Server UDP pentru citiri de senzori (protocol binar cu CRC32).
...
Datagram format (23 bytes):
  ...
  temp: temperatura în °C (float IEEE 754, big-endian)
  location: nume locație (10 chars, padding cu \0)
  crc32: checksum peste primii 19 bytes

COMPORTAMENT:
- Primește datagrame de la senzori
- Verifică CRC și validează format
- Loghează citirile valide
- Ignoră pachetele corupte (cu log warning)
- Calculează statistici (medie, min, max)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Server UDP pentru citiri de senzori"
    )
    ...
    parser.add_argument("--verbose", "-v", action="store_true", help="Afișează fiecare citire")
    parser.add_argument("--stats-interval", type=int, default=10, help="Afișează statistici la fiecare N citiri")
