#!/usr/bin/env python3
"""
Topologie Mininet – Săptămâna 11 (bază)
h1 (client) -> lb (load balancer) -> b1,b2,b3 (backends) printr-un switch s1.
...
class LBTopo(Topo):
    """
    Topologie pentru load balancing: client -> lb -> b1,b2,b3
    
    Plan IP standard WEEK 11 (10.0.11.0/24):
    ...
    def build(self, delay="0ms", loss=0, bw=None):
        ...
        # Hosts - Plan IP standard WEEK 11
...
def _cmd_bg(host, cmd, pidfile):
    # rulează în background şi salvează PID
...
def run_test(net, kit_root: Path):
    ...
    info("[TEST] Pornire load balancer...\n")
...
def main():
    ...
    try:
        ...
        if args.cli or (not args.test):
            info("\n[INFO] Intrare în CLI Mininet. Ieşiţi cu 'exit'.\n")
            CLI(net)
    finally:
        net.stop()
