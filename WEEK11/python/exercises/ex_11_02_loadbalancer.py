#!/usr/bin/env python3
"""
Exerciţiul 11.02 – Load balancer (reverse proxy) minimal, implementat în Python.

Obiectiv didactic:
- să vedem concret ce înseamnă "accept, aleg backend, forward, return response"
- să implementăm algoritmi de distribuţie (round-robin, least_conn, ip_hash)
- să înţelegem failover-ul pasiv (similar max_fails/fail_timeout din Nginx)

Atenţie:
- proxy-ul este la nivel TCP/HTTP simplificat (pentru GET/HEAD uzuale).
- nu implementează complet HTTP keep-alive/pipelining.
...
Load generator (alternativ la ab):
...
class LoadBalancer:
    ...
    def pick(self, client_ip: str) -> Optional[Backend]:
        ...
        raise ValueError(f"Algoritm necunoscut: {self.algo}")

    def mark_success(self, b: Backend) -> None:
        with self._lock:
            b.fails = 0  # resetăm la succes (simplificare)
...
def parse_backends(s: str) -> List[Backend]:
    ...
    if not out:
        raise ValueError("Lista de backends este goală.")
    return out
...
def forward_one_request(client_sock: socket.socket,
    ...
    """
    Citeşte o cerere HTTP (headers + body dacă există) şi o forwardează către backend.
    """
    ...
    lb.inc_active(b)
    try:
        # încercăm 1 backend; la eşec, încercăm încă o dată alt backend (failover simplificat)
        for attempt in (1, 2):
            try:
...
def http_get_once(url: str, timeout: float = 2.5) -> Tuple[int, float]:
    """
    Returnează (status_code, latency_s). Implementare minimală HTTP/1.1.
    """
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        try:
            ...
            status = 0
        # consumăm restul (nu ne interesează)
        while True:
            chunk = s.recv(4096)
            if not chunk:
                break
