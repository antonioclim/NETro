#!/usr/bin/env python3
"""ex_14_02.py — Harness de verificare pentru proiectul de echipă.

Funcționalități:
  - Verifică conectivitate (ping)
  - Verifică porturi TCP deschise
  - Verifică endpoint-uri HTTP
...
def check_tcp_port(host: str, port: int, timeout: int = 3) -> Dict[str, Any]:
    ...
    try:
        ...
        result["error"] = "conexiune refuzată"
    except OSError as e:
        result["error"] = str(e)
    finally:
        sock.close()
...
def run_checks(config: Dict) -> Dict[str, Any]:
    """Rulează toate verificările din configurație."""
    report = {
    ...
    for target in config.get("targets", []):
        ...
        log(f"Verificare țintă: {target_name} ({target_host})")
        ...
        for check in target.get("checks", []):
            ...
            if check_type == "ping":
                ...
                result = {
                    "type": check_type,
                    "success": False,
                    "error": f"tip verificare necunoscut: {check_type}",
                }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Harness verificare proiect S14")
    parser.add_argument("--config", required=True, help="Fișier configurare JSON")
    parser.add_argument("--out", help="Fișier output raport JSON")
...
def main() -> int:
    ...
    log(f"Încărcare configurație: {args.config}")
    config = load_config(args.config)
    
    log("Pornire verificare...")
    ...
    print("\n" + "=" * 50)
    print(f"  Verificare completă: {passed}/{total} trecute")
    print("=" * 50)
    ...
    if args.out:
        ...
        log(f"Raport salvat în: {args.out}")
    elif args.verbose:
