#!/usr/bin/env python3
"""lb_proxy.py — Load Balancer / Reverse Proxy HTTP simplu.

Funcționalități:
  - Round-robin distribuție între backends
  - Health check pasiv (marchează backend indisponibil la eroare)
  - Adaugă headere de forwarding (X-Forwarded-For, X-Real-IP)
...
class Backend:
    """Reprezintă un backend cu stare de sănătate."""
...
class LoadBalancer:
    """Load balancer round-robin cu health tracking."""
    ...
    def get_next_backend(self) -> Optional[Backend]:
        """Returnează următorul backend sănătos (round-robin)."""
        with self._lock:
            healthy_backends = [b for b in self.backends if b.healthy]
            if not healthy_backends:
                # Fallback: încearcă orice backend
    ...
    def get_stats(self) -> dict:
        """Returnează statistici pentru toate backends."""
        return {
            "backends": [
                {
                    "address": b.address,
...
class ProxyHandler(BaseHTTPRequestHandler):
    """Handler pentru proxy/load balancer."""
    ...
    def _proxy_request(self) -> None:
        """Proxy-ează cererea către un backend."""
        backend = self.lb.get_next_backend()
        if backend is None:
            self._send_error(503, "Niciun backend sănătos disponibil")
            return
        
        # Construiește URL-ul către backend
        backend_url = f"http://{backend.host}:{backend.port}{self.path}"
        
        # Copiază headerele relevante
        headers = {}
        for key, value in self.headers.items():
            if key.lower() not in ("host", "connection"):
                headers[key] = value
        
        # Adaugă headere de forwarding
        ...
        try:
            ...
            # Backend a răspuns cu eroare HTTP
            self.send_response(e.code)
            self.send_header("X-Backend", backend.address)
            self.send_header("X-Proxy-Error", "backend-http-error")
            self.end_headers()
            self.wfile.write(e.read())
            backend.mark_success()  # Backend funcționează, doar a returnat eroare
            ...
            self._send_error(502, f"Backend indisponibil: {backend.address}")
    ...
    def do_GET(self) -> None:
        if self.path == "/lb-status":
            # Endpoint special: statistici LB
            import json
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--backends",
        required=True,
        help="Lista de backends (format: host1:port1,host2:port2)"
    )
    parser.add_argument(
        "--timeout",
        type=float,
        default=5.0,
        help="Timeout pentru conexiuni backend (default: 5.0s)"
    )
...
def main() -> int:
    ...
    print(f"[proxy] Pornire load balancer pe {args.listen_host}:{args.listen_port}")
    print(f"[proxy] Backends: {[b.address for b in lb.backends]}")
    print(f"[proxy] Endpoint special: /lb-status")
